...
import os, sys, ctypes, ctypes.util, logging, mmap, struct, select, time, ffmpeg
from fcntl import ioctl
from threading import Thread
from operator import lt, gt
from PIL import Image
import zmq, json, argparse

from camera_backend import CameraCtrls, PTZController
from camera_backend import v4l2_capability, v4l2_format, v4l2_streamparm, v4l2_requestbuffers, v4l2_buffer
from camera_backend import VIDIOC_QUERYCAP, VIDIOC_G_FMT, VIDIOC_G_PARM, VIDIOC_S_PARM
from camera_backend import VIDIOC_REQBUFS, VIDIOC_QUERYBUF, VIDIOC_QBUF, VIDIOC_DQBUF, VIDIOC_STREAMON, VIDIOC_STREAMOFF
from camera_backend import V4L2_CAP_VIDEO_CAPTURE, V4L2_CAP_STREAMING, V4L2_MEMORY_MMAP, V4L2_BUF_TYPE_VIDEO_CAPTURE
from camera_backend import V4L2_PIX_FMT_YUYV, V4L2_PIX_FMT_YVYU, V4L2_PIX_FMT_UYVY, V4L2_PIX_FMT_YU12, V4L2_PIX_FMT_YV12
from camera_backend import V4L2_PIX_FMT_NV12, V4L2_PIX_FMT_NV21, V4L2_PIX_FMT_GREY
from camera_backend import V4L2_PIX_FMT_RGB565, V4L2_PIX_FMT_RGB24, V4L2_PIX_FMT_BGR24, V4L2_PIX_FMT_RX24
from camera_backend import V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_JPEG
import asyncio, zmq.asyncio, threading
...
# SDL structs and function definitions
...
SDL_SaveBMP_RW = sdl2.SDL_SaveBMP_RW
SDL_SaveBMP_RW.restype = ctypes.c_int
SDL_SaveBMP_RW.argtypes = [ctypes.POINTER(SDL_Surface), ctypes.c_void_p, ctypes.c_int]

SDL_RWFromFile = sdl2.SDL_RWFromFile
SDL_RWFromFile.restype = ctypes.c_void_p
SDL_RWFromFile.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
...
# Placeholder palettes (unchanged)
...
class V4L2Camera(Thread):
    ...
    def start_recording(self, output_file="output.mp4", framerate=30, include_audio=True):
        ...
        if not self.is_alive():
            logging.warning("Thread not started.")
            return
        ...
        try:
            ...
            if include_audio:
                ...
                out = ffmpeg.output(video_input, audio_input, output_file,
                                    vcodec='libx264', acodec='aac', pix_fmt='yuv420p', r=framerate, f='mp4', shortest=None)
            else:
                out = ffmpeg.output(video_input, output_file,
                                    vcodec='libx264', pix_fmt='yuv420p', r=framerate, f='mp4')

            self.ffmpeg_process = out.overwrite_output().run_async(pipe_stdin=True)
    ...
    def capture_loop(self):
        ...
        while not self.stopped:
            if len(poll.poll(1000)) == 0:
                logging.warning(f'{self.device}: timeout occurred')
                timeout += 1
                if timeout == 5:
                    self.pipe.write_buf(None)
                    return
            ...
            # Send frame to pipe (e.g., for display)
        ...
        # Ensure recording stops when capture loop ends
        if self.is_recording:
            self.stop_recording()
    ...
    def stop_camera(self):
        ...
        try:
            ...
            ioctl(self.fd, VIDIOC_STREAMOFF, buf_type)
        except Exception as e:
            logging.warning(f"VIDIOC_STREAMOFF failed: {e}")
        ...
        try:
            ...
            logging.warning(f"Failed to unmap buffers: {e}")
        ...
        try:
            ...
            logging.warning(f"VIDIOC_REQBUFS clear failed: {e}")

        try:
            os.close(self.fd)
        except Exception as e:
            logging.warning(f"Failed to close device: {e}")

        logging.info("Camera stopped and released successfully.")
...
class SDLCameraWindow:
    def __init__(self, device, win_width, win_height, angle, flip, colormap):
        ...
        self.rgb_surface = SDL_CreateRGBSurfaceFrom(None, width, height, 24, width * 3, 0xFF0000, 0x00FF00, 0x0000FF, 0)
        if not bool(self.rgb_surface):
            logging.error(f'SDL_CreateRGBSurfaceFrom for RGB failed: {SDL_GetError()}')
            sys.exit(1)
    ...
    def write_buf(self, buf):
        ...
        self.last_frame_ptr = ptr
        self.last_frame_format = V4L2Format2SDL(self.cam.pixelformat) if self.cam.pixelformat != V4L2_PIX_FMT_GREY else SDL_PIXELFORMAT_NV12
        self.last_frame_bytes = buf.bytesused
    ...
    def capture_image(self, timestamp=None, output_file=None, format="bmp"):
        if not hasattr(self, 'last_frame_ptr'):
        ...
        format = format.lower()
        supported_formats = {"jpeg", "jpg", "png", "bmp", "svg"}
        if format not in supported_formats:
            logging.warning(f"Unsupported format '{format}'. Defaulting to 'bmp'")
            format = "bmp"
        
        if not output_file.lower().endswith(f".{format}"):
            output_file = f"{output_file}.{format}"

        bgr_buffer = (ctypes.c_uint8 * (self.cam.width * self.cam.height * 3))()
        if self.cam.pixelformat in [V4L2_PIX_FMT_MJPEG, V4L2_PIX_FMT_JPEG]:
            SDL_ConvertPixels(self.cam.width, self.cam.height, SDL_PIXELFORMAT_RGB24, 
                            self.last_frame_ptr, self.bytesperline, 
                            SDL_PIXELFORMAT_BGR24, bgr_buffer, self.cam.width * 3)
        else:
            SDL_ConvertPixels(self.cam.width, self.cam.height, self.last_frame_format, 
                            self.last_frame_ptr, self.bytesperline, 
                            SDL_PIXELFORMAT_BGR24, bgr_buffer, self.cam.width * 3)

        img = Image.frombuffer("RGB", (self.cam.width, self.cam.height), 
                            bgr_buffer, "raw", "BGR", self.cam.width * 3)
        
        try:
            if format == "bmp":
                self.rgb_surface[0].pixels = ctypes.cast(bgr_buffer, ctypes.c_void_p)
                rw = SDL_RWFromFile(bytes(output_file, 'utf-8'), b"wb")
                if SDL_SaveBMP_RW(self.rgb_surface, rw, 1) != 0:
                    logging.error(f"Failed to save BMP image: {SDL_GetError()}")
                else:
                    logging.info(f"Image saved as {output_file}")
            elif format == "svg":
                temp_file = f"{output_file}_temp.png"
                img.save(temp_file, "PNG")
                logging.warning("SVG support requires additional vector conversion. Saved as PNG instead.")
    ...
    async def message_listener_nobusy(self, port=5555):
    ...
    def msg_handler(self, msg):
        try:
            if msg:
                ...
                if msg["action"] == "capture_image":
                    print("MSG :",msg)
                    self.capture_image(timestamp=msg['timestamp'], output_file=msg['filename'], format=msg['format'])
                if msg["action"] == "start_record":
                    print("MSG :",msg)
                    # self.record_video(msg['action'], timestamp=msg['timestamp'], output_file=msg['filename'], 
                    #                 format=msg['format'], framerate=set_framerate, resolution=set_resolution)
                    # self.cam.start_recording("video_capture.yuv")
                    self.cam.start_recording("test_video.mp4", framerate=30, include_audio=True)
                if msg["action"] == "stop_record":
                    print("MSG :",msg)
                    #self.record_video(msg['action'])
                    self.cam.stop_recording()
                #if msg["action"] == "pixelformat":
                    #self.record_video(msg['action'])
                    
                    #print("MSG :",msg)
                    # print("1",self.cam.pixelformat)
                    # self.cam.stop()
                    # self.stop_capturing()
                    # self.close()
                    
                    # if self.cam.is_running():
                    #     logging.info("Stopping camera for pixelformat change...")
                    #     self.cam.stop_camera()
                    
                    # self.cam.pixelformat=set_pixelformat(self.cam.pixelformat,msg["format"])
                    
                    # self.cam = V4L2Camera(gdevice)
                    # self.cam.pipe = self
                    # self.cam.start()

                    # print("2",self.cam.pixelformat)
                    
                    # self.cam = V4L2Camera(gdevice)
                    # self.cam.start()
                    #self.__init__(gdevice, gwidth, gheight, self.angle, self.flip, self.colormap)
                    #self.close()
        except Exception as e:
            print(e)
            pass

    def run_async_listener(callback_obj):
        asyncio.run(callback_obj.message_listener_nobusy())

    def event_loop(self):
        #socket=message_listener()
        event = SDL_Event()
        msg_thread = threading.Thread(target=self.run_async_listener, daemon=True)
        msg_thread.start()
        while SDL_WaitEvent(ctypes.byref(event)) != 0:
            ...
            if event.type == SDL_QUIT:
                ...
                if event.key.keysym.sym == SDLK_f:
                    ...
                    action = "start_record" if not self.is_recording else "stop_record"
                    self.record_video(action)
            elif event.type == SDL_MOUSEBUTTONUP and event.button.button == SDL_BUTTON_LEFT and event.button.clicks == 2:
                self.toggle_fullscreen()
...
def usage():
    print()
    print(f'Test Version - 130125\nAuthor: Biprodeep Roy\nExample:')
    print(f'  {sys.argv[0]} -d /dev/video2')
    print()


# def get_options():
#     logging.basicConfig(level=logging.INFO)  # Enable logging for feedback
#     try:
...
#     return device, width, height, angle, flip, colormap


def get_options():
    parser = argparse.ArgumentParser(description='Camera Control Script')
    parser.add_argument('-d', '--device', default='/dev/video0', help='Camera device path (e.g., /dev/video0)')
    parser.add_argument('-W', '--width', type=int, default=640, help='Camera width')
    parser.add_argument('-H', '--height', type=int, default=480, help='Camera height')
    parser.add_argument('-a', '--angle', type=float, default=0, help='Rotation angle')
    parser.add_argument('-f', '--flip', action='store_true', help='Flip image')
    parser.add_argument('-c', '--colormap', type=str, default="none",help='Colormap to apply')
    ...
    return args.device, args.width, args.height, args.angle, args.flip, args.colormap

def message_listener(port=5555):
    context = zmq.Context()
    socket = context.socket(zmq.SUB)
    try:
        socket.bind(f"tcp://*:{port}")
...
async def message_listener_nobusy(callback_obj, port=5555):
    context = zmq.asyncio.Context()
    socket = context.socket(zmq.SUB)
    socket.bind(f"tcp://*:{port}")
    socket.setsockopt_string(zmq.SUBSCRIBE, "")
    ...
    while True:
        try:
            ...
            if msg["action"] == "capture_image":
                callback_obj.capture_image(
                    timestamp=msg['timestamp'],
                    output_file=msg['filename'],
                    format=msg['format']
                )
        except json.JSONDecodeError as e:
            print(f"[WARN] Bad JSON: {e}")
...
def main(device, width, height, angle, flip, colormap):
...
def suntek_camera_init(device, width, height, angle, flip, colormap):
    sys.exit(main(device, width, height, angle, flip, colormap))
