...
import os, sys, logging, subprocess, time, zmq, json
import gi
from camera_backend import CameraCtrls, PTZHWControllers, find_by_text_id, get_devices, v4ldirs, find_idx
from interthread_signal import inter_thread
import queue  # Assuming inter_thread is a queue.Queue

inter_thread_template={"action":"","timestamp":0,"filename":"","format":"mp4"}

gi.require_version('Gtk', '3.0')
from gi.repository import Gtk, Gio, GLib, Pango, Gdk, GObject

def initiate_thread_cmd(action,timestamp,output_file,format):
    cmd_msg=inter_thread_template
    cmd_msg["action"]=action
    cmd_msg["timestamp"]=timestamp
    cmd_msg["filename"]=output_file
    cmd_msg["format"]=format
    inter_thread.put(cmd_msg)

def initiate_thread_cmd_zmq(action, timestamp, output_file, format, port=5555):
    context = zmq.Context()
    ...
    socket.send_string(json.dumps(cmd_msg))
    socket.close()
    context.term()
...
class CameraCtrlsWindow(Gtk.ApplicationWindow):
    ...
    def init_window(self):
        ...
        headerbar = Gtk.HeaderBar(title='Suntek Global Camera Application', show_close_button=True)
        ...
        def on_device_row_activated(lb, r):
            index = r.get_index()
            print(self.devices[index])
            initiate_thread_cmd("psettingchange","","",str(self.devices[index]).split('->')[-1].strip())
            self.gui_open_device(index)
            self.device_sw.get_popover().popdown()        
        self.device_lb.connect('row-activated', lambda lb, r:on_device_row_activated(lb,r))
    ...
    def init_gui_device(self):
        ...
        capture_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10, margin=10)
        stack.add_titled(capture_box, 'capture', 'Save Capture')

        capture_image_btn = Gtk.Button(label='Capture Image', tooltip_text='Take a single picture')
        capture_image_btn.connect('clicked', self.capture_image)
        capture_box.pack_start(capture_image_btn, False, False, 0)

        self.record_video_btn = Gtk.Button(label='Start Recording', tooltip_text='Start recording a video')
        self.record_video_btn.connect('clicked', self.toggle_recording)
        capture_box.pack_start(self.record_video_btn, False, False, 0)
    ...
    def capture_image(self, widget):
        if not self.device:
            self.notify("No camera selected.", timeout=5)
            return

        timestamp = time.strftime("%Y%m%d_%H%M%S")
        output_file = f"capture_{timestamp}"
        initiate_thread_cmd_zmq("capture_image",timestamp,output_file,"jpeg")
    ...
    def start_recording(self):
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        output_file = f"video_{timestamp}"
        try:
            initiate_thread_cmd_zmq("start_record",timestamp,output_file,"mp4")
            self.is_recording = True
            self.record_video_btn.set_label("Stop Recording")
            self.notify(f"Recording started: {output_file}", timeout=5)
            GLib.timeout_add(500, self.check_recording)
        except Exception as e:
            self.notify(f"Error starting recording: {str(e)}", timeout=5)

    def stop_recording(self):
        if self.is_recording:
            try:
                initiate_thread_cmd_zmq("stop_record","","","mp4")
                self.notify("Recording stopped.", timeout=5)
            except subprocess.TimeoutExpired:
                self.notify("Recording forcefully stopped due to timeout.", timeout=5)
            finally:
                self.is_recording = False
                self.record_video_btn.set_label("Start Recording")
                self.capture_process = None

    def check_recording(self):
        if self.capture_process and self.is_recording:
...
class CameraCtrlsApp(Gtk.Application):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, application_id='hu.irl.suntek', **kwargs)
        self.window = None
    ...
    def do_startup(self):
        ...
        #Start monitoring inter_thread queue
        #GLib.timeout_add(100, self.check_inter_thread)
    ...
    def on_about(self, action, param):
        ...
        about.set_program_name('Suntek Global Camera Settings')
        about.set_authors(['Suntek Global'])
        about.set_artists(['Suntek Global'])
        about.set_copyright('Copyright © 2025 Suntek Global')
        about.set_license_type(Gtk.License.LGPL_3_0)
        about.set_logo_icon_name('hu.irl.suntek')
        about.set_website("https://suntekglobal.com/")
        about.set_website_label('STAR')
    ...
    def on_quit(self, action, param):
        ...
        # self.quit()  # Ensure the application quits
    ...
    def check_inter_thread(self):
        try:
            # Check queue without blocking
            msg = inter_thread.get_nowait()
            if msg.get("action") == "pixelformat":
                print("Received pixelformat action, stopping application...")
                self.on_quit(None, None)  # Trigger quit
...
def suntek_camera_settings_init():
    app = CameraCtrlsApp()
    app.run()
